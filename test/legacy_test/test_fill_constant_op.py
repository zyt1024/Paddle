...
from paddle.base import Program, core, program_guard, framework
...
class TestFillConstantComplex64Op(OpTest):
    def setUp(self):
        '''Test fill_constant op with default value'''
        self.op_type = "fill_constant"
        self.init_dtype()
        self.init_shape()
        self.init_value()
        # self.python_api = paddle.full(shape=self.shape,fill_value=self.value, dtype=self.dtype)
        self.python_api = wrap_fill_wrapper(self.dtype)
        self.inputs = {}
        self.attrs = {'shape': self.shape, 'value': self.value, 'dtype': framework.convert_np_dtype_to_dtype_(
            self.dtype
        )}
        self.outputs = {'Out': np.full(self.shape, self.value)}
